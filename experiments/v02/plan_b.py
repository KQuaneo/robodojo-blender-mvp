...
def stage(t, pairs=None, arm=None):
    if 215 <= t <= 240 and (arm==1 or (pairs and any('Right_' in str(x) for x in pairs))):
...
def main():
    ...
    report = dict(id=args.trial, seed=trial['seed'], status='planning',
                  ...
                  method='B_local_retarget_v02', checks=0, no_fallback=True)
    ...
    reference = json.loads((ROOT / manifest['reference_dir'] / 'joint_trace.json').read_text())
    cal = json.loads((ROOT / manifest['reference_dir'] / 'grasp_calibration.json').read_text())
    ...
    for t in times:
        ...
        if np.linalg.norm(delta) > 0:
            ...
            for seed in ([q[0],previous[0]] if previous is not None else [q[0]]):
                cache_key=(target.tobytes(),np.asarray(seed,dtype=float).tobytes())
                if cache_key in ik_cache:
                    cached,err=ik_cache[cache_key];candidate=cached.copy();solver_cache_hits+=1
                else:
                    candidate, err = p.single_ik(target, robots[0], seed)
                    ik_cache[cache_key]=(candidate.copy(),err);solver_calls+=1
    ...
    report['solver_calls']=solver_calls
    report['solver_exact_input_cache_hits']=solver_cache_hits
    ...
    if np.linalg.norm(offset_b)+np.linalg.norm(offset_p)>0:
        ...
        for t in times:
            for angle,spec in zip(new_q[t][0],b.ARM_JOINTS):
                obj=robots[0]['joints'][spec.name]
                obj.rotation_quaternion=Euler(spec.rpy,'XYZ').to_quaternion() @ Quaternion(Vector(spec.axis),float(angle))
                obj.keyframe_insert('rotation_quaternion',frame=t)
        for spec in b.ARM_JOINTS:
            for fc in curves(robots[0]['joints'][spec.name]).values():
    ...
    analytical=motion.rate_audit(actual_curves)
    report['analytic_continuity']=analytical
    if not analytical['passed']:
        failure=analytical['violations'][0]
        finish(stage(failure['start_frame'],arm=0 if failure['arm']=='Left' else 1),failure=dict(type='analytic_continuity',**failure))
        return
    ...
    for t in samples:
        ...
        if previous is not None:
            ...
            if max(float(np.max(np.abs(q[i]-previous[i]))) for i in (0,1)) > .48*(t-previous_t)+1e-5:
                which=int(np.argmax([np.max(np.abs(q[i]-previous[i])) for i in (0,1)]))
                finish(stage(t,arm=which),failure=dict(frame=t,type='continuity',arm=which,rad_per_frame=rate))
                return
        previous,previous_t = q,t
        mats = dict(initial)
        for i in (0,1):
