"""v0.2 fresh execution, original predicates and key-aware independent audit."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--manifest',required=True);ap.add_argument('--trial',required=True);ap.add_argument('--output',required=True);ap.add_argument('--preflight',action='store_true');ap.add_argument('--plan-dir')
    ...
    plan=json.loads((plan_dir/'plan.json').read_text()) if plan_dir else None
    if plan:
        assert plan['outcome']=='planned' and plan['manifest_sha256']==digest(args.manifest)
        for name,sha in plan['files'].items():assert digest(plan_dir/name)==sha
        assert Path(bpy.data.filepath).resolve()==(plan_dir/'planned.blend').resolve()
    started=time.time();scene=bpy.context.scene;scene.frame_set(1)
    fcs=motion.rotation_curves();continuity=motion.rate_audit(fcs)
    def mat(name):return np.array(bpy.data.objects[name].evaluated_get(bpy.context.evaluated_depsgraph_get()).matrix_world)
    base=ROOT/manifest['reference_dir'];qtrace=json.loads(((plan_dir or base)/'joint_trace.json').read_text());cal=json.loads((base/'grasp_calibration.json').read_text())
    ...
    pan_original=mat('broom_shovel');pan_to_flange=np.array(plan['pan_to_flange']) if plan else np.linalg.inv(pan_original)@flange(0,270)
    ...
    for name,delta in ([] if plan else layout['offsets_m'].items()):
    ...
    audit_grid=motion.audit_times(fcs,last) if last else []
    for moment in audit_grid:
        f=int(moment);fraction=moment-f
        if True:
        ...
        if moment%100==0:print('AUDIT',f,'events',len(events),flush=True)
    support=[]
    if last:
    ...
    qualified=bool(continuity['passed'] and task and supported and dynamic and limits and not events and max_attachment<1e-5 and max_tilt<=2 and max_speed<=120 and all(g['passed'] for g in gate_results))
    ...
    result={'id':args.trial,'seed':layout['seed'],'status':'completed','termination_reason':reason,'qualified':qualified,
        ...
        'continuity':continuity,'audit_grid_frames':audit_grid,
        'reference_version':'v0.2','plan_sha256':digest(plan_dir/'plan.json') if plan else None,
        ...
        'limitations':['Frozen sweep; local acquisition retargeting only when plan supplied','Gated kinematic attachment, not force closure','Sampled robot collision check, not continuous-time proof']}
