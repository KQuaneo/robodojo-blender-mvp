"""Execute a validated B plan in a fresh process; preserve legacy acceptance checks."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--manifest',required=True);ap.add_argument('--trial',required=True);ap.add_argument('--output',required=True);ap.add_argument('--preflight',action='store_true');ap.add_argument('--plan-dir',required=True)
    ...
    plan_dir=Path(args.plan_dir);plan=json.loads((plan_dir/'plan.json').read_text())
    assert plan['outcome']=='planned' and plan['id']==args.trial
    assert plan['manifest_sha256']==digest(args.manifest)
    ...
    base=ROOT/'output_smooth';qtrace=json.loads((plan_dir/'joint_trace.json').read_text());cal=json.loads((base/'grasp_calibration.json').read_text())
    ...
    pan_to_flange=np.array(plan['pan_to_flange'])
    # Saved plan already contains initialization; do not apply offsets twice.
    ...
    result={'id':args.trial,'seed':layout['seed'],'status':'completed','termination_reason':reason,'qualified':qualified,
        ...
        'plan_sha256':digest(plan_dir/'plan.json'),'method':'B_local_retarget_v1',
        'limitations':['Local scene-conditioned acquisition replanning with world-fixed sweep; no global search','Gated kinematic attachment, not force closure','Sampled robot collision check, not continuous-time proof']}
